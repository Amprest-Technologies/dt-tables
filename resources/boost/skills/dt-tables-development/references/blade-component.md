...
|-----------|------|-------------|
| `id` | `string` | Unique identifier — **must match** the `key` field in its `resources/data-tables/{key}.json` file |
...
| `dtt-title="key"` | Column key string | Enables column-specific filtering/sorting. The key must match a column key in that table's `resources/data-tables/{key}.json` |
...
1. **Searchable/filterable columns** — the key links the column to its `resources/data-tables/{key}.json` search configuration
...
2. A column `key` in that table's `resources/data-tables/{key}.json` configuration
...
2. Looks up `resources/data-tables/{key}.json` for a matching `key` to load column configuration (search types, themes, buttons)
...
4. Injects an inline `<script type="module">` that initializes the DataTable with:
   ...
   - Column config from `resources/data-tables/{key}.json`
